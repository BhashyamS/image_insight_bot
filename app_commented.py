# Imports: built-in Python libraries used for file handling, JSON parsing, environment variables, dates, and type hints.
import io
import json
import os
...
# Third-party libraries: pandas for tables, Streamlit for UI, PIL for image handling.
import pandas as pd
import streamlit as st
from PIL import Image, ImageDraw

# Optional Plotly imports. The app still runs if Plotly is not installed; charts are skipped.
try:
...
# Optional Gemini SDK imports. If missing, the app shows a clear install error when analysis runs.
try:
...
# Streamlit page configuration: title, icon, wide layout, and expanded sidebar.
st.set_page_config(
...
# Custom CSS injected into Streamlit to make the app look like a polished business product.
st.markdown(
    """
<style>
...
# Default response object. Used before analysis or if the AI/API fails, so the UI never breaks from missing keys.
...
# Prompt schema: tells Gemini exactly what JSON structure to return. This is critical for predictable UI rendering.
SCHEMA = """
...
# Reads the Gemini API key from Streamlit secrets first, then from local environment variables.
...
# Displays an API-key input only if no secret/environment key is found.
...
# Cleans model output in case Gemini wraps JSON in markdown fences like ```json.
...
# Converts Gemini text into a Python dictionary, with fallback handling for imperfect JSON.
...
# Forces score values into a safe 0-100 integer range for metrics and charts.
...
# Normalizes Gemini output so every expected field exists and has the right type.
...
# Core AI function: sends the uploaded image and business prompt to Gemini, then returns normalized JSON.
def analyze_image(image_bytes: bytes, mime: str, model: str, business_mode: str, detail: str, key: str) -> Dict[str, Any]:
    ...
    client = genai.Client(api_key=key)  # Creates the Gemini client using the API key.
...
{SCHEMA}
"""
    image_part = types.Part.from_bytes(data=image_bytes, mime_type=mime)  # Converts uploaded image bytes into Gemini-compatible input.
    response = client.models.generate_content(
        model=model,
        contents=[prompt, image_part],
        config=types.GenerateContentConfig(response_mime_type="application/json", temperature=0.2),
    )
    return normalize(parse_json(response.text))  # Parse JSON, fill missing fields, and return safe output.


# Draws AI-estimated bounding boxes on the image using normalized 0-1000 coordinates.
...
# Converts numeric scores into plain-English labels shown beside Streamlit metrics.
...
# Renders the six marketing scorecards and progress bars.
...
# Builds a Plotly bar chart for the marketing-readiness scores.
...
# Builds a horizontal bar chart showing best-fit marketing channels.
...
# Renders AI-generated tags as pill-shaped labels using custom CSS.
def render_tags(tags: List[str]) -> None:
    if tags:
        st.markdown(" ".join([f"<span class='tag'>{tag}</span>" for tag in tags[:18]]), unsafe_allow_html=True)


# Main report renderer: turns the Gemini JSON into tabs, cards, charts, tables, and recommendations.
...
# If multiple images are uploaded, this compares their scores in one dashboard.
...
# Main Streamlit app flow: layout, sidebar setup, file upload, analysis loop, and export.
def main() -> None:
    st.markdown(
        """
<div class="hero">
...
""",
    ...
    with st.sidebar:
        ...
        key = sidebar_api_key()  # Gets API key from secrets/env/sidebar input.
    ...
    if st.button("Generate Business Intelligence Report", type="primary", use_container_width=True):
        ...
        for uploaded in files:
            image_bytes = uploaded.getvalue()  # Reads uploaded file into bytes for PIL and Gemini.
            img = Image.open(io.BytesIO(image_bytes))  # Opens bytes as an image object for display and box drawing.
            with st.spinner(f"Analyzing {uploaded.name} as a business visual asset..."):
                try:
                    result = analyze_image(image_bytes, uploaded.type or "image/jpeg", model, business_mode, detail, key)  # Sends image to Gemini for analysis.
        ...
        # Download button exports all image analyses as one JSON report.
        st.download_button(
...
# Python entry point. Runs main() only when this file is executed directly.
